...
const Shuffle = ({
  ...
  useGSAP(
    () => {
      ...
      const removeHover = () => {
        if (hoverHandlerRef.current && elRef.current) {
      ...
      const armHover = () => {
        if (!triggerOnHover || !elRef.current) return;
        removeHover();
        const handler = () => {
        ...
        elRef.current.addEventListener("mouseenter", handler);
      };
  ...
  return (
    <Tag ref={elRef} className={classes} style={commonStyle}>
      {text}
    </Tag>
  );
};
